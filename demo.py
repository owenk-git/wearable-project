...
def rotate_imu_data(data, x_rot, y_rot, z_rot):
    """
    Rotates IMU data to match the paper's coordinate system.
    
    Parameters:
        data (np.ndarray): shape (N, 3), columns are [x, y, z]

    Returns:
        np.ndarray: rotated data, shape (N, 3)
    """
    # Define rotation (you can tweak these if needed)
    r_z = R.from_euler('z', z_rot, degrees=True)  # Clockwise 180° about Z
    r_x = R.from_euler('x', x_rot, degrees=True)   # Clockwise 90° about X
    r_y = R.from_euler('y', y_rot, degrees=True)    # Counter-clockwise 90° about Y

    # Final rotation matrix
    T = r_y.as_matrix() @ r_x.as_matrix() @ r_z.as_matrix()

    # Apply the rotation to each row (transpose is important!)
    rotated = data[..., :3] @ T.T
    norm = torch.norm(rotated, p='fro', dim=-1, keepdim=True)
    data = torch.cat([rotated, norm], dim=-1)
    return data
